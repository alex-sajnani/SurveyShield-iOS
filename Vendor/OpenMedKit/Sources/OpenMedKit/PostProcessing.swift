...
public enum PostProcessing {
    ...
    public static func decodeEntities(
        ...
        for token in tokens {
            ...
            // Models are trained with labels only on the first subword of each
            // word, so continuation pieces often re-predict "B-" instead of "I-".
            // A piece that starts exactly where the previous one ended is mid-word
            // and must extend the current entity, whatever its BIO prefix.
            let isWordContinuation = currentLabel == entityType && token.startOffset == currentEnd
            if currentLabel != entityType || (isBeginning && !isWordContinuation) {
                if let curLabel = currentLabel {
                    entities.append(
                        makeEntity(
                            label: curLabel,
